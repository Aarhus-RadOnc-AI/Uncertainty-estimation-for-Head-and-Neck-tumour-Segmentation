...
def load_snapshot_model_and_checkpoint_files(folder, folds=None, snapshots=['600', '700', '800', '900', '1000'], mixed_precision=None):
    """
    used for if you need to ensemble the five models from epoch [600, 700, 800, 900, 1000]. This will restore the model from the
    checkpoint in epoch 600, load all parameters of the five epochs in ram and return both. This will allow for fast
    switching between parameters (as opposed to loading them form disk each time).
    ...
    trainer = restore_model(join(folds[0], "model_epoch_%s.model.pkl" % snapshots[0]), fp16=mixed_precision)
    trainer.output_folder = folder
    trainer.output_folder_base = folder
    trainer.update_fold(0)
    trainer.initialize(False)
    all_best_model_files = [join(i, "model_epoch_%s.model" % snaps) for i in folds for snaps in snapshots]
    print("using the following model files: ", all_best_model_files)
    all_params = [torch.load(i, map_location=torch.device('cpu')) for i in all_best_model_files]
    return trainer, all_params
