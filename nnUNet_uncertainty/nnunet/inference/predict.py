...
from nnunet.training.model_restore import load_model_and_checkpoint_files,load_snapshot_model_and_checkpoint_files
...
def predict_cases(model, list_of_lists, output_filenames, folds, save_npz, num_threads_preprocessing,
    ...
    # ! make change if load different snapshot models  
    num_snaps = len(snapshots)
    if num_snaps == 0 :
        print("loading ordinary parameters for folds,", folds)
        trainer, params = load_model_and_checkpoint_files(model, folds, mixed_precision=mixed_precision,
                                                        checkpoint_name=checkpoint_name)
    else:
        print("loading snapshot parameters for folds,", folds)
        trainer, params = load_snapshot_model_and_checkpoint_files(model, folds, snapshots, mixed_precision=mixed_precision)                         
    ...
    for preprocessed in preprocessing:
        ...
        if num_mc_dropout == 1:
            ...
            # ! enable for mc dropout, both here, add argument if mc_drop: self.network.eval()
            print("predicting for MC_dropout", output_filename)
            trainer.load_checkpoint_ram(params[0], False)
            print("predicting for MC_dropout", output_filename, 'random number: 0')
            softmax = trainer.predict_preprocessed_data_return_seg_and_softmax_dropout(
                d, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
                step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
                mixed_precision=mixed_precision)[1]
            for num in range(num_mc_dropout-1):
                print("predicting for MC_dropout", output_filename, 'random number: ', num)
                softmax += trainer.predict_preprocessed_data_return_seg_and_softmax_dropout(
                    d, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
                    step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
                    mixed_precision=mixed_precision)[1]
            ...
            for p in params[1:]:
                trainer.load_checkpoint_ram(p, False)
                for num in range(num_mc_dropout):
                    print("predicting for MC_dropout", output_filename, 'random number: ', num)
                    softmax += trainer.predict_preprocessed_data_return_seg_and_softmax_dropout(
                        d, do_mirroring=do_tta, mirror_axes=trainer.data_aug_params['mirror_axes'], use_sliding_window=True,
                        step_size=step_size, use_gaussian=True, all_in_gpu=all_in_gpu,
            ...
            if len(params) > 1 or num_mc_dropout>1:
                softmax /= len(params*num_mc_dropout)
...
def predict_from_folder(model: str, input_folder: str, output_folder: str, folds: Union[Tuple[int], List[int]],
                        ...
                        segmentation_export_kwargs: dict = None, disable_postprocessing: bool = False,
                        num_mc_dropout: int = 1, snapshots: list = []):
    """
    ...
    if mode == "normal":
        ...
        return predict_cases(model, list_of_lists[part_id::num_parts], output_files[part_id::num_parts], folds,
                             ...
                             disable_postprocessing=disable_postprocessing,num_mc_dropout = num_mc_dropout, 
                             snapshots=snapshots)
    elif mode == "fast":
        if overwrite_all_in_gpu is None:
            all_in_gpu = False
...
if __name__ == "__main__":
    ...
    # ! add for mc dropout 2021/10/10
    num_mc_dropout = args.num_mc_dropout
    ...
    predict_from_folder(model, input_folder, output_folder, folds, save_npz, num_threads_preprocessing,
                        ...
                        step_size=step_size,num_mc_dropout=num_mc_dropout,snapshots=snapshots)
