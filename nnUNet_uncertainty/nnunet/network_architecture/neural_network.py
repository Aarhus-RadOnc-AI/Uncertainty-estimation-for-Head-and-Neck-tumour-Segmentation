...
class SegmentationNetwork(NeuralNetwork):
    ...
    def predict_3D(self, x: np.ndarray, do_mirroring: bool, mirror_axes: Tuple[int, ...] = (0, 1, 2), 
                   do_gaussian_noise: bool = False, use_sliding_window: bool = False,
            ...
            with torch.no_grad():
                if self.conv_op == nn.Conv3d:
                    if use_sliding_window:
                        res = self._internal_predict_3D_3Dconv_tiled(x, step_size, do_mirroring, mirror_axes,do_gaussian_noise, patch_size,
                                                                     regions_class_order, use_gaussian, pad_border_mode,
                                                                     pad_kwargs=pad_kwargs, all_in_gpu=all_in_gpu,
                                                                     verbose=verbose)
                    else:
                        res = self._internal_predict_3D_3Dconv(x, patch_size, do_mirroring, mirror_axes,do_gaussian_noise, regions_class_order,
                                                               pad_border_mode, pad_kwargs=pad_kwargs, verbose=verbose)
                elif self.conv_op == nn.Conv2d:
                    if use_sliding_window:
                        res = self._internal_predict_3D_2Dconv_tiled(x, patch_size, do_mirroring, mirror_axes,do_gaussian_noise, step_size,
                                                                     regions_class_order, use_gaussian, pad_border_mode,
    ...
    def predict_2D(self, x, do_mirroring: bool, mirror_axes: tuple = (0, 1, 2), do_gaussian_noise: bool = False, 
                   use_sliding_window: bool = False, step_size: float = 0.5, patch_size: tuple = None, 
                   regions_class_order: tuple = None, use_gaussian: bool = False, pad_border_mode: str = "constant",
                   pad_kwargs: dict = None, all_in_gpu: bool = False,
                   verbose: bool = True, mixed_precision: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        with context():
            with torch.no_grad():
                if self.conv_op == nn.Conv2d:
                    if use_sliding_window:
                        res = self._internal_predict_2D_2Dconv_tiled(x, step_size, do_mirroring, mirror_axes, do_gaussian_noise, patch_size,
                                                                     regions_class_order, use_gaussian, pad_border_mode,
                                                                     pad_kwargs, all_in_gpu, verbose)
                    else:
                        res = self._internal_predict_2D_2Dconv(x, patch_size, do_mirroring, mirror_axes, do_gaussian_noise, regions_class_order,
    ...
    def _internal_predict_3D_3Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, do_gaussian_noise: bool, mirror_axes: tuple,
            ...
            for y in steps[1]:
                ...
                for z in steps[2]:
                    ...
                    predicted_patch = self._internal_maybe_mirror_and_pred_3D(
                        data[None, :, lb_x:ub_x, lb_y:ub_y, lb_z:ub_z], mirror_axes, do_mirroring, do_gaussian_noise,
    ...
    def _internal_predict_2D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool, do_gaussian_noise: bool,
        ...
        predicted_probabilities = self._internal_maybe_mirror_and_pred_2D(data[None], mirror_axes, do_mirroring, do_gaussian_noise,
                                                                          None)[0]
    ...
    def _internal_predict_3D_3Dconv(self, x: np.ndarray, min_size: Tuple[int, ...], do_mirroring: bool, do_gaussian_noise: bool,
                                    mirror_axes: tuple = (0, 1, 2), regions_class_order: tuple = None,
                                    pad_border_mode: str = "constant", pad_kwargs: dict = None,
                                    verbose: bool = True) -> Tuple[np.ndarray, np.ndarray]:
        """
        ...
        predicted_probabilities = self._internal_maybe_mirror_and_pred_3D(data[None], mirror_axes, do_mirroring, do_gaussian_noise,
                                                                          None)[0]
    ...
    def _internal_maybe_mirror_and_pred_3D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
                                           do_mirroring: bool = True, do_gaussian_noise: bool = False, 
                                           mult: np.ndarray or torch.tensor = None) -> torch.tensor:
        assert len(x.shape) == 5, 'x must be (b, c, x, y, z)'

        #! 2021/06/05 Add TTA of making gaussian noise with argument "do_gaussian_noise"
        ...
        #! 2021/06/05 Add TTA of making gaussian noise
    ...
    def _internal_maybe_mirror_and_pred_2D(self, x: Union[np.ndarray, torch.tensor], mirror_axes: tuple,
        ...
        for m in range(mirror_idx):
            ...
            if m == 4:  #! 2021/06/05 Add TTA of making gaussian noise
                noise = (0.1**0.5) * torch.randn_like(x)  # Generate Gaussian noise with variance of 0.1
                x_noisy = x + noise  # Add noise to the input
                pred = self.inference_apply_nonlin(self(x_noisy))
                result_torch += 1 / num_results * pred
    ...
    def _internal_predict_2D_2Dconv_tiled(self, x: np.ndarray, step_size: float, do_mirroring: bool, do_gaussian_noise: bool, mirror_axes: tuple,
                                          patch_size: tuple, regions_class_order: tuple, use_gaussian: bool,
                                          pad_border_mode: str, pad_kwargs: dict, all_in_gpu: bool,
                                          verbose: bool) -> Tuple[np.ndarray, np.ndarray]:
        # better safe than sorry
        ...
        for x in steps[0]:
            ...
            for y in steps[1]:
                ...
                predicted_patch = self._internal_maybe_mirror_and_pred_2D(
                    data[None, :, lb_x:ub_x, lb_y:ub_y], mirror_axes, do_mirroring,do_gaussian_noise,
                    gaussian_importance_map)[0]
    ...
    def _internal_predict_3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool, do_gaussian_noise: bool,
        ...
        for s in range(x.shape[1]):
            pred_seg, softmax_pres = self._internal_predict_2D_2Dconv(
                x[:, s], min_size, do_mirroring, mirror_axes, do_gaussian_noise, regions_class_order, pad_border_mode, pad_kwargs, verbose)
    ...
    def predict_3D_pseudo3D_2Dconv(self, x: np.ndarray, min_size: Tuple[int, int], do_mirroring: bool, do_gaussian_noise: bool,
                                   mirror_axes: tuple = (0, 1), regions_class_order: tuple = None,
        ...
        for s in range(extra_slices, data.shape[1] - extra_slices):
            ...
            pred_seg, softmax_pres = \
                self._internal_predict_2D_2Dconv(d, min_size, do_mirroring, mirror_axes, do_gaussian_noise,
    ...
    def _internal_predict_3D_2Dconv_tiled(self, x: np.ndarray, patch_size: Tuple[int, int], do_mirroring: bool,
        ...
        for s in range(x.shape[1]):
            pred_seg, softmax_pres = self._internal_predict_2D_2Dconv_tiled(
                x[:, s], step_size, do_mirroring, mirror_axes, do_gaussian_noise, patch_size, regions_class_order, use_gaussian,
