...
class DataLoader3D(SlimDataLoaderBase):
    ...
    def determine_shapes(self):
        ...
        if isfile(self._data[k]['data_file'][:-4] + ".npy"):
            case_all_data = np.load(self._data[k]['data_file'][:-4] + ".npy", self.memmap_mode).astype(np.float16)  # ! force to run ft16 20220920
        else:
            case_all_data = np.load(self._data[k]['data_file'])['data'].astype(np.float16) # ! force to run ft16 20220920
